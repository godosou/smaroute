...
m_origin(origin), m_destination(destination),m_proposalProbabilities(proposalProbabilities), m_detour_cost_scale(
...
double MHPathProposal::transitionLogProb(MHPath& from_route, MHPath& to_route,
	...
	if (splice == true) {
		...
		return log(0.5) + log(m_proposalProbabilities.find(to_route.getNodeB())->second);
	} else {
		/*
...
MHTransition<MHPath> MHPathProposal::newTransition(MHPath& from_route) {
	...
	if (m_rnd->nextDouble() < 0.5 && from_route.isSpliceable()) {
        //		splice_count++;
