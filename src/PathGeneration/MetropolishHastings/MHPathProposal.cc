					...
					return log(0.5)
							+ log(
									from_route.getInsertProbs(
											m_detour_cost_scale)[to_route.getNodeB()]
											+ 1.0
...
MHPath MHPathProposal::newInitialState() {
	...
	MHPoints points = drawPoints(new_path.nbrOfNodes()-1, m_rnd);
	return MHPath(new_path, points, m_router);
}

const patNode* MHPathProposal::drawInsertNode(
		const map<const patNode*, double>& probas) {
...
//		DEBUG_MESSAGE("splice");
		...
		if (insertNode == NULL) {
			to_route = from_route;
			fwdLogProb = transitionLogProb(from_route, to_route, false);
			bwdLogProb = transitionLogProb(to_route, from_route, false);
		} else {
			...
			if (proposal_route.containLoop()) {
				...
				fwdLogProb = transitionLogProb(from_route, to_route, true);
				bwdLogProb = transitionLogProb(to_route, from_route, true);
			}
		}
	} else {
		/*
...
//		DEBUG_MESSAGE("shuffle");
		to_route = from_route;
		to_route.setPoints(drawPoints(to_route.nbrOfNodes()-1, m_rnd));
