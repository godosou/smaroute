...
void patMonteCarlo::generateNextPath(patNetwork* theNetwork, patOd aOd, patError*& err) {
  theNetwork->generateMonteCarloCosts(&theNormal, stderr, true, err) ;
  if (err != NULL) {
  ...
  if (success) {
    ...
    if (found == listOfPaths.end()) {
      listOfPaths[theShortest] = pair<double,unsigned long>(0,1) ;
    } 
    else {
...
void patMonteCarlo::setStandardError(double s) {
  stderr = s ;
}
