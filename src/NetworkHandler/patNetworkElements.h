...
public:
	 ...
	 * @return True if adding is successful; False otherwise;
	 */
	bool addWay(patWay* the_way,list<unsigned long> the_list_of_nodes_ids, patError*& err);
	bool addProcessedWay(patWay& the_way,list<unsigned long> the_list_of_nodes_ids,
			unsigned long source, unsigned long target, patError*& err);
	/**
	 ...
	 */
	patArc* addArc(const patNode* upNode, const patNode* downNode, patWay* theWay, patError*& err);

	patPublicTransportSegment* addPTSegment(patPublicTransportSegment* ptSeg,patError*& err);
	/**
	...
	const patArc* findArcByNodes(const patNode* up_node, const patNode* down_node) const;
	void computeLength();

	set<const patNode*> getNearbyNode(const patGeoCoordinates& coords, double distance,int count) const;
protected:
