...
//
	if (patNBParameters::the()->databaseHost != "") {
		m_network_elements.readNetworkFromPostGreSQL(bb);
	} else {
		m_network_elements.readNetworkFromOSMFile(

		patNBParameters::the()->OsmNetworkFileName, bb);
	}
...
void patNetworkEnvironment::computeGeneralizedCost(
