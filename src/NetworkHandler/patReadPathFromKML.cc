...
patReadObservationFromKML::patReadPathFromKML(string a_m_file_name, patNetworkElements* a_network) :
m_file_name(a_m_file_name),
m_network(a_network) {

}
 patReadObservationFromKML::parseFile() {
    xml::tree_parser parser(m_file_name.c_str());

    if (!parser) {
        WARNING("Error while parsing " << m_file_name);
