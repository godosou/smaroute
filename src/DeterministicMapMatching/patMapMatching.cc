 ...
 */
set<patNode*> patMapMatching::getNearestNodes(patGpsPoint* theGpsPoint, double searchRadius, patError*& err) {
 ...
 * @return
 */
bool patMapMatching::stageMapMatchingGenerateSubRoutesWithGaps(list<patMapMatchingRoute>* subRoutesWithGaps, patError* err) {
                ...
                if (!(currRoute.getRouteSize() == 1 && reachedStartOfArc(lastArc, currGps))) {
                    ...
                    if (reachedEndOfArc(lastArc, currGps, currRoute.getLastGps(), &currRoute)) {
                        ...
                        for (set<unsigned long>::iterator outIter = lastNode->userSuccessors.begin();
                                ...
                                ++outIter)
                        {
                            if (*outIter != lastArc->m_up_node_id && *outIter!=lastArc->m_down_node_id) {
 ...
 */
bool patMapMatching::reachedEndOfArc(patArc* arc, patGpsPoint* gps, patGpsPoint* prevGps, patMapMatchingRoute* route) {
    map<char*, double> dist = gps->distanceTo(theNetwork, arc);
    if (dist["position"] == 1) {
    ...
    double headingDiff = fabs(gpsHeading - arc->m_attributes.heading);
    if(!(headingDiff<=85.0||fabs(360.0 - headingDiff) <= 85.0)){
            DEBUG_MESSAGE("leave arc by angle"<<gpsHeading<<","<<arc->m_attributes.heading<<"-"<<headingDiff<<","<<fabs(360.0 - headingDiff) );
            return true;
    }
    double distance = 0.0;
 ...
 * @return
 */
bool patMapMatching::reachedStartOfArc(patArc* arc, patGpsPoint* gps) {
    map<char*, double> dist = gps->distanceTo(theNetwork, arc);
...
patPathJ patMapMatching::joinChosenRoutes(list<patMapMatchingRoute>* subRoutes,
    ...
    if (subRoutes->size() == 0) {
        ...
        for (; itSR != subRoutes->end(); ++itSR) {
            ...
            patNode* lastNode = theNetwork->getNodeFromUserId(lastSubRoute.getLastArc()->m_down_node_id);
            if (lastNode == NULL) {
                err = new patErrNullPointer("patNode");
                WARNING(err->describe());
                return path;
            ...
            map<double, set<list<unsigned long> > > paths;
            for (vector<patNode*>::iterator eIter = listOfEndNodes.begin();
                ...
                for (vector<patNode*>::iterator sIter = listOfStartNodes.begin();
                    ...
                    if (startNode != endNode) {
                        list<unsigned long> seg = destTree.getShortestPathFrom(startNode->internalId, err);

                        if (err != NULL) {
                            WARNING(err->describe());
                            return path;
                        }
                        double cost = destTree.labels[startNode->internalId];

                        if (paths.find(cost) == paths.end()) {
                            set<list<unsigned long> > ele;
                            ele.insert(seg);
                            paths[cost] = ele;
                        }
                        else {
            ...
            if (paths.empty()) {
                ...
                set<list<unsigned long> > shortestPaths = paths.begin()->second;
                list<unsigned long> shortestPath = *(shortestPaths.begin());

                if (shortestPath.front() == theNetwork->getInternalNodeIdFromUserId(lastSubRoute.getFirstArc()->m_up_node_id) ||
                        shortestPath.back() == theNetwork->getInternalNodeIdFromUserId(lastSubRoute.getLastArc()->m_down_node_id)) {
                    ...
                    for (list<unsigned long>::iterator nIdIter = shortestPath.begin();
