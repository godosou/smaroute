	...
	try {
		...
		if (argc == 1) {
			patNBParameters::the()->readFile(
					"/Users/jchen/Documents/Project/biogeme/newbioroute/config.xml",
					err);
		...
		*network = *network_environment.getNetwork(CAR);
		 ...
		 */
		boost::mt19937 rng(patNBParameters::the()->RANDOMSEED_ELEMENT);
		MHPathGenerator path_generator(rng);
		path_generator.setNetwork(network);
		path_generator.setPathWriter(&path_writer);

		const patNode* origin_node  = network_environment.getNetworkElements().getNode(263297446);
		const patNode* destination_node  = network_environment.getNetworkElements().getNode(263291704);
