...
void patPathSampling::run(unsigned long algoName,		patError*& err){
	switch(algoName){
 ...
 */
double patPathSampling::getShortestPathCostFrom(const unsigned long aId,patError*& err){
	...
	double cost=destTree.labels[aId];
	//DEBUG_MESSAGE(cost);
	if( cost<100000.0){
...
double patPathSampling::computePathProbaForRandomWalk(patPathJ* thePath,patError*& err){
	double logPathProba = 0.0;
	...
	for(list<patArc*>::iterator arcIter = thePath->getArcList()->begin();
		...
		double theShortestPathCost=patMaxReal;
		if (tmpNode == theOd->getDestination()){

			dest_count+=1.0;
			//DEBUG_MESSAGE("dest"<<dest_count);
			logPathProba+=log(pow(0.5,dest_count));
			for (set<unsigned long>::iterator iter =  tmpNode->userSuccessors.begin();
				iter!=tmpNode->userSuccessors.end();
				++iter){
					patArc* tmpArc = theNetwork->getArcFromNodesUserId(tmpNode->userId,*iter);
					if ( tmpArc==NULL||tmpArc->m_name=="M1" || tmpArc->m_attributes.priority<4 || tmpArc->m_attributes.priority>14){
						continue;
					}
					double spCostDN = getShortestPathCostFrom(theNetwork->getInternalNodeIdFromUserId(tmpArc->m_down_node_id),err);
		...
		for (set<unsigned long>::iterator iter =  tmpNode->userSuccessors.begin();
			...
			patArc* tmpArc = theNetwork->getArcFromNodesUserId(tmpNode->userId,*iter);
			if ((*tmpArc).m_name=="M1" || (*tmpArc).m_attributes.priority<4 || (*tmpArc).m_attributes.priority>14) {
				continue;
			}
			//theOd.orig = theNetwork->getNodeFromUserId(tmpArc->downNodeId);
			double spCost = getShortestPathCostFrom(theNetwork->getNodeFromUserId(tmpArc->m_down_node_id)->internalId,err);

			if (err!=NULL || spCost==patMaxReal) {
				DEBUG_MESSAGE("no shortest path to dest");
				continue;
			}
			double theRatio = theShortestPathCost/(spCost+tmpArc->getLength());
		//	DEBUG_MESSAGE(theRatio<<"="<<theShortestPathCost<<"/("<<spCost<<"+"<<tmpArc->getLength()<<")");
			...
			double kuma = patPower(theRatio*10.0,kumaA);
...
void patPathSampling::newSampledPath(patPathJ aPath, double proba){
	patPathJ* newPathPt = newPath(aPath);
	rwLogProba[newPathPt]=proba;
}

void patPathSampling::addPath(patPathJ aPath,patError*& err){
	double sampleProba=computePathProbaForRandomWalk(&aPath,err);
...
pair<patArc*,double> patPathSampling::getNextArcInRandomWalk(const patNode* fromNode,patUnixUniform* randomNumbersGenerator,patError*& err){
	...
	if(fromNode!=theOd->getDestination()){
		...
		for (set<unsigned long>::iterator iter =  fromNode->userSuccessors.begin();
	...
	if (err != NULL) {
		...
		WARNING(err->describe()) ;
		return pair<patArc*,double>(NULL,0.0) ;
	}
	...
	for (set<unsigned long>::iterator iter =  fromNode->userSuccessors.begin();
		...
		//theOd.orig = theNetwork->getNodeFromUserId(tmpArc->downNodeId);
		double spCost = getShortestPathCostFrom(theNetwork->getInternalNodeIdFromUserId(tmpArc->m_down_node_id),err);
		if (err != NULL || spCost==patMaxReal) {
			DEBUG_MESSAGE("not valid outgoing link");
			continue;
		}

		double theRatio = theShortestPathCost/(spCost+tmpArc->generalizedCost);
		if(theRatio >1.0){
			stringstream str ;
	...
	if(proba.size()==0|| total==0.0){
		DEBUG_MESSAGE("error ");
		return pair<patArc*,double>(NULL,0);
	}

	unsigned long selectId = patDiscreteDistribution(&proba,randomNumbersGenerator)();

	return pair<patArc*,double>(theArcs[selectId],proba[selectId]);
}
...
//		DEBUG_MESSAGE(logProba);
		if (currentNode==theOd->getDestination()){
			...
			patError* err(NULL) ;
			 ...
			 pair<patArc*, double> result = getNextArcInRandomWalk(currentNode, randomNumbersGenerator, err);
...
//			DEBUG_MESSAGE("add a arc"<<*(result.first)<<","<<result.second);
			...
			logProba += log(result.second);
			 ...
			 currentNode =theNetwork->getNodeFromUserId(result.first->m_down_node_id);
			if (currentNode == NULL) {
...
void patPathSampling::getMultiplePathsWithRandomWalk(unsigned long nbrOfPaths,
								unsigned long nbrOfTrials,
								patUnixUniform* randomNumbersGenerator,
								patError*& err)
{
	for(unsigned long iter = 0;
...
bool patPathSampling::isAddedPath(patPathJ* aPath){
...
void patPathSampling::computeEPS( patError*& err){
	...
	map<patArc*, double> EPM; //expansion M for chosen path
...
void patPathSampling::computePathSizes( patError*& err){
	...
	double pathSizeGamma = patNBParameters::the()->pathSizeGamma;

	map<patArc*,unsigned long > arcOverlap;
...
void patPathSampling::randomWalk(patError*& err){
	unsigned long maxNumberOfGeneratedPaths = patNBParameters::the()->maxNumberOfGeneratedPaths;
	unsigned long maxTrialsForRandomWalk = patNBParameters::the()->maxTrialsForRandomWalk;
...
double patPathSampling::getPathEPS(patPathJ* aPath,patError*& err){
...
double patPathSampling::getPathPS(patPathJ* aPath,patError*& err){
...
double patPathSampling::getPathRwLogProba(patPathJ* aPath,patError*& err){
...
unsigned long patPathSampling::getPathReplicates(patPathJ* aPath,patError*& err){
