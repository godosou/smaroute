...
void patTripParser::setMapBounds(double minLat, 
			      double maxLat, 
			      double minLon, 
			      double maxLon) {
...
bool patTripParser::addPoint(const patGpsPoint theGpsPoint) {
...
void patTripParser::genPath(){
	set<unsigned long> theO = gpsSequence[0].getStartNodes(theNetwork);
 ...
 */
  ...
  kml<<"</Style>"<<endl;
	   ...
	   kml<<(buffer )<<endl;
		...
		for(map<patArc*,double>::iterator aIter = linkDDR->begin();aIter!=linkDDR->end();++aIter){
			 kml << "            <Placemark>" << endl ;
			   		kml<<"<TimeStamp>"<<endl;
  		kml<<"	<when>"<<endl;
...
vector<double> patTripParser::genPathDDR(){
