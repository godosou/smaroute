...
patTripParser::patTripParser(patTraveler* theTraveler, patULong& theTripId,
...
void patTripParser::setMapBounds(patReal minLat, patReal maxLat, patReal minLon,
		patReal maxLon) {
...
void patTripParser::presentSAResults(patString SAType,
		map<patString, vector<patReal> > values) {
...
void patTripParser::writeToKML(patString fileName) {
	...
	for (map<patOdJ*, map<patPathJ*, vector<patReal> > >::iterator odIter =
		...
		for (map<patPathJ*, vector<patReal> >::iterator pathIter =
...
void patTripParser::genOd() {
	...
	//leastDistance = 10000;
	for (map<patArc*, patReal>::const_iterator iter =
...
void patTripParser::genAssocDomain(
		map<patArc*, set<patULong> >* associateDomain) {
	for (int i = 0; i < gpsSequence.size(); i++) {
