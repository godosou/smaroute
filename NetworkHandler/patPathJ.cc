...
ostream& operator<<(ostream& str, const patPathJ& x)  {
	str<<"<"<<x.listOfArcs.front()->getUpNode()->getUserId();
...
bool patPathJ::readShpFile(string file_path,
	...
	for (int i = 0; i < line_counts; ++i) {
		...
		const list<patArc*>* the_arc_list = a_way->getListOfArcs();
...
bool patPathJ::exportShpFiles( string file_path, patNetworkElements* network, patError*& err){
	if(listOfArcs.size()!=modes.size()){
		stringstream str ;
	...
	list<patArc*>::iterator arc_iter = listOfArcs.begin();
	list<TransportMode>::iterator mode_iter=modes.begin();
	vector<short>::iterator change_point_iter = change_points.begin();
	short k=0;
	...
	int mode_dbf_id = DBFAddField( path_dbf_handler, "mode",
			FTInteger, 2, 0 );

	int source_dbf_id = DBFAddField( path_dbf_handler, "source",
			FTInteger, 10, 0 );
	int target_dbf_id = DBFAddField( path_dbf_handler, "target",
			FTInteger, 10, 0 );

	//Loop until the end of the path and write each arc and change point.
	double up_node_x = (*arc_iter)->getUpNode()->getGeoCoord().longitudeInDegrees;
	double up_node_y = (*arc_iter)->getUpNode()->getGeoCoord().latitudeInDegrees;

	while(arc_iter!=listOfArcs.end()){

		//Get vertices.
		double down_node_x = (*arc_iter)->getDownNode()->getGeoCoord().longitudeInDegrees;
		double down_node_y = (*arc_iter)->getDownNode()->getGeoCoord().latitudeInDegrees;
		...
		SHPObject* path_shp_object  = SHPCreateSimpleObject( SHPT_ARC,2,
					    padfX, padfY,NULL);
		int object_number = SHPWriteObject(shp_file_handler, -1, path_shp_object);
		...
		//Get change point.
		if(change_point_iter!=change_points.end() && *change_point_iter==k){
			double chg_points_padfX[1] = {(*arc_iter)->getUpNode()->getGeoCoord().longitudeInDegrees};
			double chg_points_padfY[1] = {(*arc_iter)->getUpNode()->getGeoCoord().latitudeInDegrees};
