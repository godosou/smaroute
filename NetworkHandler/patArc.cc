...
patArc::patArc(patULong theId, patNode* theUpNode, patNode* theDownNode,
		patString theName, struct arc_attributes theAttr, patError*& err) :
		userId(theId), internalId(patBadId), name(theName), attributes(theAttr), upNode(
				theUpNode), downNode(theDownNode), generalizedCost(0.0) {
...
patArc::patArc(patULong theId, patNode* theUpNode, patNode* theDownNode) :
		userId(theId), upNode(theUpNode), downNode(theDownNode), generalizedCost(
				0.0) {
	length = theUpNode->geoCoord.distanceTo(theDownNode->geoCoord);
	calHeading();
}
patArc::patArc(patULong theId, patNode* theUpNode, patNode* theDownNode,
		...
		userId(theId), internalId(patBadId), name(theName), upNode(theUpNode), downNode(
				theDownNode) {
	if ((theUpNode == NULL) || (theDownNode == NULL)) {
		err = new patErrNullPointer("patNode");
		WARNING(err->describe());
...
patReal patArc::calHeading() {
	...
	patReal denumerator = sin(lat2) * cos(lat1)
...
patReal patArc::calHeading(patNetwork* theNetwork, patError*& err) {
	...
	if ((theUpNode == NULL) || (theDownNode == NULL)) {
		...
		err = new patErrNullPointer("patNode");
		DEBUG_MESSAGE(
				"node" << upNodeId << "," << theUpNode << "-" << downNodeId
						<< "," << theDownNode);
		WARNING(err->describe());
		return -1.0;
	}
...
ostream& operator<<(ostream& str, const patArc& x) {
	str << "Arc " << x.userId << " [" << x.name << "](" << x.upNode->getUserId()
			<< "->" << x.downNode->getUserId() << ")";
	str << " length=" << x.length << "m; ";
	return str;
}
patULong patArc::computeTurn(patArc* downArc, patNetwork* theNetwork) {
	...
	patULong left = 0;
	patULong right = 0;
	patReal headingChange = 180.0
			- (downArc->attributes.heading - attributes.heading);
	headingChange =
	...
	for (set<patULong>::iterator iter1 = successors->begin();
		...
		patReal h1 = 180.0
				- (downStream->attributes.heading - attributes.heading);
