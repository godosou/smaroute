...
patReal baseFunction_simple_diff(patReal* k, size_t dim, void* params) {
	...
	patReal d_curr_2 = pow(fp->network.d_curr, 2)
			+ pow((k[0] - fp->network.e_d_curr) * fp->network.l_curr, 2);
	patReal d_prev_2 = pow(fp->network.d_prev, 2)
...
void patArcTransition::setNetworkParams(patNetwork* theNetwork,
	...
	map<char*, patReal> distanceToArc = currGpsPoint->distanceTo(theNetwork,
			path.back());
	p.d_curr = distanceToArc["ver"];
	p.e_d_curr = currGpsPoint->calPerpendicularFootOnArc(distanceToArc);

	p.l_prev = path.front()->getLength();
	map<char*, patReal> distanceToArc_Prev = prevGpsPoint->distanceTo(
...
patReal patArcTransition::calProbability_raw(patNetwork* theNetwork,
...
list<patReal> patArcTransition::calProbability(patNetwork* theNetwork,
		struct gps_params GP) {
	list<patReal> values;
	values.push_back(
...
patReal baseFunction_MM_2modes(patReal* k, size_t dim, void* params) {

	struct base_params * fp = (struct base_params *) params;
	return fp->TM->integral(k[0], k[1], k[2]) * fp->MM->integral(k[0], k[1]);
}

patReal patArcTransition::calMultiModalProbability(patNetwork* theNetwork,
		struct gps_params GP) {
	if (path.isUniModal()) {
		return 0.0;
	}
...
patReal patArcTransition::calProbability(patString algoType,
...
vector<patReal> patArcTransition::dealIntersections(patNetwork* theNetwork) {
	...
	for (; arcIter != listOfArcs->end(); arcIter++) {
		patReal t_w = calWaitingTime(prevArc, const_cast<patArc*>(*arcIter),
				theNetwork, err);
		if (t_w >= 0.0) {
...
patReal patArcTransition::calSignalWaiting(patArc* upArc, patArc* downArc,
	...
	patULong left = 0;
	patULong right = 0;
	patReal headingChange = 180.0 - (downArc->attributes.heading)
			- upArc->attributes.heading;
	headingChange =
	...
	for (set<patULong>::iterator iter1 = successors->begin();
		...
		patReal h1 = 180.0
				- (downStream->attributes.heading - upArc->attributes.heading);
...
patReal patArcTransition::getValue(patString type, patNetwork* theNetwork,
		struct gps_params GP) {
	if (type == "raw") {
		if (valueRaw < 0.0) {
