...
patArcTransition::patArcTransition(
		...
		value(-1.0), m_value_raw(-1.0), m_value_simple(-1.0), m_path(listOfArcs), m_prev_gps_point(
				curr_GpsPoint), m_curr_gps_point(prev_GpsPoint) {
...
void patArcTransition::setNetworkParams(struct network_params& p) {
	...
	map<char*, double> distance_to_arc = m_curr_gps_point->distanceTo(path.back());
	...
	map<char*, double> distance_to_arc_Prev = m_prev_gps_point->distanceTo(path.front());
	p.d_prev = distance_to_arc_Prev["ver"];
	p.e_d_prev = m_prev_gps_point->calPerpendicularFootOnArc(distance_to_arc_Prev);
...
double patArcTransition::calProbability_raw(struct gps_params GP) {
	...
	//map<char*, double> distanceToArc = m_curr_gps_point->distanceTo( path.back());
	//m_value_raw = m_curr_gps_point->measureDDR(distanceToArc);
	m_value_raw =
			(m_value_raw < 0.0) ?
					m_curr_gps_point->getArcDDRValue(path.back()) :
					m_value_raw;
...
double patArcTransition::calMultiModalProbability(struct gps_params GP) {
	if (path.isUniModal()) {
		return 0.0;
	}
...
double patArcTransition::calProbability(patString algoType, struct gps_params GP) {
	if (algoType == "raw") {
...
*/	
	 ...
	 */
		...
		xl = new double[2];
		xu = new double[2];
		struct link_ddr_range curr_ddr_range = m_curr_gps_point->getLinkDDRRange(
				getLastArc());
		struct link_ddr_range prev_ddr_range = m_prev_gps_point->getLinkDDRRange(
				getFirstArc());
...
patGpsPoint* patArcTransition::getm_prev_gps_point() {
	return m_prev_gps_point;
}
patGpsPoint* patArcTransition::getm_curr_gps_point() {
