...
map<char*, patReal> patPathProbaAlgoV2::calDistancePoint2Arc(patGpsPoint* thePoint,patArc* theArc){
	patGeoCoordinates* upNodeGeoCoord = &(baseNetwork->getNodeFromUserId(theArc->upNodeId)->geoCoord);
	patGeoCoordinates* downNodeGeoCoord = &(baseNetwork->getNodeFromUserId(theArc->downNodeId)->geoCoord);
...
patReal patPathProbaAlgoV2::calSignalWaiting(patArc* upArc,patArc* downArc,patNode* intersection){
	...
	patULong left=0;
	patULong right=1;
	patReal headingChange = downArc->attributes.heading - upArc->attributes.heading;
