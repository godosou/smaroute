...
bool patShortestPathGeneral::buildShortestPathTree(patNode* root_node,
		double ceil) {
	set<patNode*> root_nodes;
	root_nodes.insert(root_node);
	return buildShortestPathTree(root_nodes, ceil);
}

bool patShortestPathGeneral::buildShortestPathTree(set<patNode*> root_nodes,
		patGpsDDR* gps_ddr, set<patArc*> ddr_arcs, double ceil) {
	...
	while (!list_of_nodes.empty()) {
		...
		for (set<patRoadBase*>::const_iterator outgoing_road_iter =
			...
			if (gps_ddr != NULL) {
				...
				set<patArc*> temp_arcs = (*outgoing_road_iter)->getAllArcs();
				...
				if (down_node_label
						> shortest_path_tree.labels[node_to_process] + road_cost) {
