...
public:
	 ...
	 */
	patULong getWaySize();
	patNode* getNode(patULong node_id);
	patArc* getArc(patULong arc_id);
	patWay* getWay(patULong way_id);
	virtual ~patNetworkElements();
protected:


	map<patULong, patNode> nodes;/**< The map of nodes. Key is the node osm id, value is the node object*/
	map<patULong, patArc> arcs;/**< The map of arcs. Key is the node id (generated by the program), value is the arc object*/
	map<patULong, patWay> ways;/**< The map of nodes. Key is the way osm id, value is the way object*/
private:
