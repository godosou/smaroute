...
TransportMode patReadPathFromShp::readTransportMode(patString mode_string) {
	...
	if (mode_string == "BUS") {
		...
		WARNING(
				"not recognized transport mode" << mode_string<<", use CAR instead.");
...
void patReadPathFromShp::readSequence(DBFHandle& file_handler) {
	...
	for (int i = 0; i < line_counts; ++i) {
		int sequence = DBFReadIntegerAttribute(file_handler, i,
				sequence_id_index);
...
bool patReadPathFromShp::read(patMultiModalPath* path, string file_path,
	...
	//a path should contain more than one edge in order to determine the direction.
	int line_counts = DBFGetRecordCount(file_handler);
	if (line_counts == 0) {
		WARNING("empty path.");
		return false;
	} else if (line_counts == 1) {
		WARNING("only one arc, can not determine direction");
		return false;
	}
	...
	readSequence(file_handler);//Read the sequence of arcs.

	//Read edges and directions;

	int last_edge = DBFReadIntegerAttribute(file_handler, sequence_list[1],
			edge_id_index);
	int last_source = DBFReadIntegerAttribute(file_handler, sequence_list[1],
			source_id_index);
	int last_target = DBFReadIntegerAttribute(file_handler, sequence_list[1],
			target_id_index);
	string last_mode_string = DBFReadStringAttribute(file_handler,
			sequence_list[1], mode_index);
	...
	for (int i = 2; i <= line_counts; ++i) {
		int current_edge = DBFReadIntegerAttribute(file_handler,
				sequence_list[i], edge_id_index);
		int current_source = DBFReadIntegerAttribute(file_handler,
				sequence_list[i], source_id_index);
		int current_target = DBFReadIntegerAttribute(file_handler,
				sequence_list[i], target_id_index);
		string mode_string = DBFReadStringAttribute(file_handler,
				sequence_list[i], mode_index);
	...
	if (edges_forward_direction.size() != tmp_modes.size()
			|| edges_forward_direction.size() != tmp_ways.size()) {
	...
	for (int i = 0; i < edges_size; ++i) {
		...
		const vector<const patArc*>* the_arc_list = way->getArcListPointer(forward);
		DEBUG_MESSAGE("way size: " << the_arc_list->size()<<", forward direction: "<<forward);
		for (vector<const patArc*>::const_iterator arc_iter = the_arc_list->begin();
