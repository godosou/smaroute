...
int main(int argc, char *argv[]) {
  ...
  unsigned long seed = patNBParameters::the()->randomSeed;
// seed =timeSeed;
	patError* err(NULL) ;
...
*/
	...
	for(list<unsigned long>::iterator uIter = lrp.begin();
		uIter!=lrp.end();
		++uIter){
	...
	for(unsigned long i=0;i<patNBParameters::the()->simNumberOfTrips;++i){
		...
		//dir<<"speed "<<speed;
			...
			double old = patNBParameters::the()->pointProbaPower;
			...
			double cdf = patNBParameters::the()->selectPathCdfThreshold;
			//theReadSample.readGpsTrack(&gpsSequence,++theTripId,lrp);
