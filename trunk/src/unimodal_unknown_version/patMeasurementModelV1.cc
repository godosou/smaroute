...
double patMeasurementModelV1::integral(double ell_curr, double ell_prev){
	double d_curr_2 = 	pow(np.d_curr,2)+pow((ell_curr - np.e_d_curr) * np.l_curr,2);	
	double d_prev_2 = pow(np.d_prev,2)+pow((ell_prev- np.e_d_prev) * np.l_prev,2);
