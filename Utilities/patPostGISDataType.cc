...
pair<patReal, patReal> patPostGISDataType::PointToLonLat(
	...
	smatch what;
	patReal lon = patMaxReal;
	patReal lat = patMaxReal;
	...
	return pair<patReal, patReal>(lon, lat);
}
list<patULong> patPostGISDataType::IntArrayToULongList(patString str) {
	sregex time = sregex::compile("(\\d+)");
	int const subs[] = { 1 };
	list<patULong> long_list;
