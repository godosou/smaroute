...
set<patULong> patPathDevelop::genEndNodes(){
	...
	for(map<patULong, map<patPathJ,patReal> > ::iterator iter1 = pathSet.begin();
	iter1!=pathSet.end();
	++iter1){
...
void patPathDevelop::appendSeg(patGpsPoint* gpsPoint,map<patULong,set<patPathJ> >* segSet,set<patArc*>* inheritDDR){
	...
	++iter1){
		...
		++iter2){
			if(inheritDDR->find(const_cast<patPathJ*>(&(iter2->first))->back())!=inheritDDR->end()){
				patReal length = const_cast<patPathJ*>(&(iter2->first))->computePathLength();
				minPathLength = (length < minPathLength)?length:minPathLength;
				newPathSet->insert(pair<patPathJ,patReal>(iter2->first,length));
			}
		}
	
	}
	
		set<patArc*> activeDomain = gpsSequence.back()->selectDomainByCdf(baseNetwork);;
	for(map<patULong, map<patPathJ,patReal> >::iterator iter3 = pathSet.begin();
	iter3!=pathSet.end();
	++iter3){
		
		map<patULong,set<patPathJ > >::iterator segFound = segSet->find(iter3->first);
		
		if(segFound!=segSet->end() && !segFound->second.empty()){
			//DEBUG_MESSAGE(iter3->first<<", "<<iter3->second.size()<<", "<<segFound->second.size());
			unionTwoDomains(&minPathLength, newPathSet,&(iter3->second),&(segFound->second),&activeDomain);
		}
	}
	/*
	...
	*/
	...
	if(newPathSet->size() > patNBParameters::the()->minGeneratedInterMediatePath){
...
void patPathDevelop::unionTwoDomains(patReal* minLength, map<patPathJ,patReal >* newPathSet,
		...
		++iter1){
			for(set<patPathJ >::iterator iter2= newSegSet->begin();
				...
				if(aL->size()>1 && activeDomain->find(aL->front()) == activeDomain->end()){
...
void patPathDevelop::selectPathsByPdf(list<patPathJ*>* preSelected,map<patPathJ,patReal>* newPathSet){
	...
	for(map<patPathJ,patReal>::iterator iter1= newPathSet->begin();
			iter1!=newPathSet->end();
			++iter1){
		if(iter1->second * patNBParameters::the()->selectPathInversePercent >= totalProba){
	...
	if(tmpSet.empty()){
		...
		for(map<patPathJ,patReal>::iterator iter1= newPathSet->begin();
			...
			if(tmpSet.find(const_cast<patPathJ*>(&(iter1->first)))==tmpSet.end()){
				newPathSet->erase(iter1++);
			}
			else{
				iter1++;
...
void patPathDevelop::selectPathsByCdf(list<patPathJ*>* preSelected,map<patPathJ,patReal>* newPathSet){
...
//		DEBUG_MESSAGE("proba: "<<ddrPathIter->first<<", number: "<<ddrPathIter->second.size()<<", cdf: "<<cdfProba/totalProba);
	...
	if( ddrPathIter == ddrPathSet.begin()){
		...
		DEBUG_MESSAGE(tmpSet.size()<<" out of "<<newPathSet->size()<<" paths are remained by cdf criteria");
...
void patPathDevelop::selectPathsByNumber(list<patPathJ*>* preSelected, map<patPathJ,patReal>* newPathSet){
map<patReal,set<patPathJ*> > ddrPathSet;
	/*
...
*/
	...
	for(patULong i=0;i<best;++i){
		if(ddrPathIterBack!=ddrPathSet.begin()){
			ddrPathIterBack--;
			tmpPathSet.insert(ddrPathIterBack->second.begin(),ddrPathIterBack->second.end());
		}
	...
	*/
	for(map<patPathJ,patReal>::iterator pIter = newPathSet->begin();
		...
		if(tmpPathSet.find(const_cast<patPathJ*>(&(pIter->first))) == tmpPathSet.end()){
...
void patPathDevelop::calPathDDR(map<patPathJ,patReal>* newPathSet){
	...
	for(map<patPathJ,patReal>::iterator iter= newPathSet->begin();
			iter!=newPathSet->end();
			++iter){
		patPathProbaAlgoV4 pathProba(const_cast<patPathJ*>(&(iter->first)),&gpsSequence,baseNetwork,arcTranSet,originalGpsPoints);
...
//		DEBUG_MESSAGE("path DDR"<<ddr);
		if( patNBParameters::the()->algoInSelection == "dist" || patNBParameters::the()->algoInSelection == "mm"){
			ddr = -ddr;
		}
		iter->second = ddr;	
	}
...
list<patPathJ*> patPathDevelop::selectPathByImportantDDR(map<patPathJ,patReal>* newPathSet){
	...
	for(set<patArc*>::iterator impIter = impDDR.begin();
		...
		for(map<patPathJ,patReal>::iterator pathIter = newPathSet->begin();
				pathIter != newPathSet->end();
				++pathIter){
			if(pathIter->first.isLinkInPath(*impIter)){
//				DEBUG_MESSAGE(pathIter->second)
				pathViaImpDDR.push_back(const_cast<patPathJ*>(&(pathIter->first)));
...
set<patArc*>  patPathDevelop::selectImportantDDR(){
	...
	if (patNBParameters::the()->selectImportantDDRCdf==1.0) {
		...
		for (map<patArc*,patReal>::iterator ddrIter=linkDDR->begin(); ddrIter!=linkDDR->end(); ++ddrIter) {
 ...
 patReal sumTotal = gpsSequence.back()->calDDRSum();
	...
	while (selectedDDRSum/sumTotal < patNBParameters::the()->selectImportantDDRCdf || 
		gpsSequence.back()->getLinkDDR()->size() < patNBParameters::the()->minDomainSize) {
	
		patULong selectedArcIndex=discreteDraw();
		pair<set<patArc*>::iterator,bool> insertResult=importantDDR.insert(ddrArcs[selectedArcIndex]);
...
void patPathDevelop::newInitPath(list<patULong>* pathTemp,map<patPathJ,patReal >*  newPathSet){
	...
	patPathJ newPath;
		...
		for(;nIter!=pathTemp->end();++nIter){
			...
			patArc* currArc = baseNetwork->getArcFromNodesUserId(prevNode,currNode);
			if(currArc == NULL){
				return;
			}
		...
		patReal ddr = newPath.computePointDDRRaw(gpsSequence.back()->getLinkDDR());
...
patReal patPathDevelop::calLengthCeil(){
	return  patNBParameters::the()->pathLengthCeil * calAverageSpeed() * (gpsSequence.back()->getTimeStamp()-gpsSequence.front()->getTimeStamp());
}

patReal patPathDevelop::lastStage(patSample* theSample,patObservation* generatedObservation){
		//DEBUG_MESSAGE("all paths"<<pathSet.size());
		
	for(map<patULong, map<patPathJ,patReal> > ::iterator destIter = pathSet.begin();
				destIter!=pathSet.end();
				++destIter){
		for(map<patPathJ,patReal>::iterator pathIter = destIter->second.begin();
				pathIter != destIter->second.end();
				++pathIter){
		...
		patPathProbaAlgoV4 pathProba(pathPointer,&gpsSequence,baseNetwork,arcTranSet,originalGpsPoints);
		
		patReal value1 = pathProba.run(patNBParameters::the()->algoInSelection);
		//	patReal value1 = 0.0;
			patReal value2 = 0.0;
			patReal value3=0.0;
		...
		DEBUG_MESSAGE("old value:"<<pathIter->second<<patNBParameters::the()->algoInSelection<<value1<<", simple:"<<value2<<", raw:"<<value3);
		}
	}
